...
- Node.js >= 18
- `tsx` (đã có trong devDependencies)
...
```bash
npx tsx scripts/inspect-db.ts --<command> [args...]
```
...
# Batch
npx tsx scripts/inspect-db.ts --tables
```
...
# Batch
npx tsx scripts/inspect-db.ts --schema posts
```
...
# Batch
npx tsx scripts/inspect-db.ts --select users 10
npx tsx scripts/inspect-db.ts --select todos 3
```
...
# Batch
npx tsx scripts/inspect-db.ts --where posts userId = 1
npx tsx scripts/inspect-db.ts --where todos completed = true
npx tsx scripts/inspect-db.ts --where comments postId = 5
```
...
# Batch
npx tsx scripts/inspect-db.ts --find users id 3
npx tsx scripts/inspect-db.ts --find posts userId 1
```
...
# Batch
npx tsx scripts/inspect-db.ts --search posts laboriosam
npx tsx scripts/inspect-db.ts --search users Graham
```
...
# Batch
npx tsx scripts/inspect-db.ts --count comments
```
...
# Batch
npx tsx scripts/inspect-db.ts --relationships
```
...
npx tsx scripts/inspect-db.ts --settings

# Batch (hiện rõ giá trị — cẩn thận khi share output)
npx tsx scripts/inspect-db.ts --settings --reveal
```
...
# Batch
npx tsx scripts/inspect-db.ts --settings redis
npx tsx scripts/inspect-db.ts --settings redis --reveal
```
...
# Batch (mặc định ẩn)
npx tsx scripts/inspect-db.ts --setting REDIS_HOST
npx tsx scripts/inspect-db.ts --setting APP_SECRET

# Batch (hiện rõ)
npx tsx scripts/inspect-db.ts --setting APP_SECRET --reveal
```
...
```bash
db> help
npx tsx scripts/inspect-db.ts --help
```
...
```bash
npx tsx scripts/inspect-db.ts --schema users
npx tsx scripts/inspect-db.ts --schema settings
```
...
```bash
# Tìm user theo email
npx tsx scripts/inspect-db.ts --find users email Sincere@april.biz

# Xem tất cả posts của user id 1
npx tsx scripts/inspect-db.ts --where posts userId = 1

# Tìm todos chưa hoàn thành
npx tsx scripts/inspect-db.ts --where todos completed = false

# Tìm comments chứa từ "est"
npx tsx scripts/inspect-db.ts --search comments est
...
# Tìm settings public
npx tsx scripts/inspect-db.ts --where settings isPublic = true

# Tìm settings theo key
npx tsx scripts/inspect-db.ts --find settings key RATE_LIMIT_ENABLED
```
...
```bash
DATABASE_URL="file:./production.db" npx tsx scripts/inspect-db.ts --tables

# Hoặc dùng SQLite TCP
DATABASE_URL="libsql://your-db.turso.io" npx tsx scripts/inspect-db.ts --tables
```
