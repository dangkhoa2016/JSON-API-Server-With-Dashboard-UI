...
├── db/
│   ├── schema.js               # Drizzle schema — 7 tables (users, posts, comments, albums, photos, todos, settings)
│   ├── relations.ts            # Drizzle relations — users → posts → comments, users → albums → photos, etc.
│   ├── seed.js                 # Fetch data from JSONPlaceholder and insert via Drizzle
│   ├── seed-settings.js        # Seed settings table from .env variables
│   ├── seed-admin.js           # Hash admin password with Argon2 and store in settings
