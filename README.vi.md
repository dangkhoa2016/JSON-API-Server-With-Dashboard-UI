...
- **Bộ test được kiểm soát coverage** — **69 file test** ở cả backend và frontend với ngưỡng coverage vitest (80% statements/functions/lines, 75% branches). Integration tests chạy với SQLite thật + HTTP; component tests dùng `@vue/test-utils` với jsdom; E2E browser tests dùng Playwright.
...
| `yarn test:coverage`    | Chạy tests với coverage (ngưỡng: 80/75/80/80)         |
...
│   └── __tests__/              # Bộ test backend (27 file)
├── db/
│   ├── schema.js               # Schema Drizzle (7 bảng)
│   ├── relations.ts            # Quan hệ giữa các bảng
│   ├── seed.js                 # Nạp dữ liệu từ JSONPlaceholder
│   ├── seed-settings.js        # Nạp settings
│   ├── seed-admin.js           # Nạp thông tin admin
...
│   └── __tests__/              # Bộ test frontend (41 file)
...
Dự án áp dụng **ngưỡng coverage** qua vitest (statements 80%, branches 75%, functions 80%, lines 80%). Bộ test bao gồm:
...
- **Database tests** (2 file, môi trường `node`): Kiểm thử cấu hình Drizzle, chuẩn bị schema, và seeding cơ sở dữ liệu.
- **Scripts tests** (1 file, môi trường `node`): Kiểm thử công cụ kiểm tra cơ sở dữ liệu tương tác.
...
Xem [manual/README.vi.md](manual/README.vi.md) để biết chi tiết các script và cách sử dụng.
