    ...
    <div class="flex items-start justify-between gap-4">
      <div class="flex-1 min-w-0">
        <div class="flex items-center gap-2">
          ...
          <span
            v-if="setting.value === '********'"
            class="text-xs px-1.5 py-0.5 rounded bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400"
            >sensitive</span
          >
        </div>
        <p
        ...
        <div class="mt-2">
          <div
            v-if="editingValue !== undefined && isAdmin"
            class="bg-muted dark:bg-muted/50 rounded-lg p-3 -mx-1"
          >
            <div class="flex items-center gap-2">
              <div class="relative flex-1">
                <Input
                  ...
                  @update:model-value="
                    emit('update:editingValue', setting.key, $event)
                  "
                />
                <Button
                  v-if="setting.value === '********'"
            ...
            <div class="flex items-center gap-2 mt-3">
              ...
              <Button
                size="sm"
                variant="outline"
                class="text-gray-700 border-gray-300 hover:text-red-600 hover:bg-red-50 hover:border-red-300 dark:text-gray-300 dark:border-gray-600 dark:hover:text-red-400 dark:hover:bg-red-900/20 dark:hover:border-red-800"
                @click="emit('cancel', setting.key)"
                >Cancel</Button
              >
            </div>
          </div>
