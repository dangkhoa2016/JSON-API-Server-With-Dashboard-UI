...
const parentSlots = inject<Record<string, (scope: Record<string, unknown>) => unknown>>('resource-parent-slots', {})
...
function renderCustomSlot(key: string, value: unknown, update: (v: unknown) => void) {
...
<template>
  <Dialog :model-value="modelValue" @update:model-value="emit('update:modelValue', $event)">
    <div class="space-y-4">
      <h2 class="text-lg font-semibold">{{ mode === 'create' ? 'Create' : 'Edit' }} {{ title }}</h2>
      <div v-for="field in fields" :key="field.key" class="space-y-1">
        ...
        <component
          ...
          :is="renderCustomSlot(field.key, formData[field.key], (v: unknown) => setFormField(field.key, v))"
        />
