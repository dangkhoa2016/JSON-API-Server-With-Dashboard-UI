...
interface ListQuery {
  data: Ref<
    | { data: Array<{ id: number; [key: string]: unknown }>; total: number }
    | undefined
  >;
...
export function useResourceCrud(
  ...
  type CreateInput = Parameters<typeof create.mutate>[0];
  type UpdateInput = Parameters<typeof update.mutate>[0];
  ...
  function handleUpdate(id: number, data: Record<string, unknown>) {
    update.mutate({ id, data } as UpdateInput, {
      onSuccess: () => {
        list.refetch();
        toast.success(title, {
          description: `${singular} updated successfully.`,
        });
      },
      onError: (err: { message: string }) => {
        toast.error("Failed", { description: err.message });
