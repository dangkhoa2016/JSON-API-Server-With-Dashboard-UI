...
function verifyToken() {
  ...
  verifyPromise = trpcClient.query('admin.auth.verify')
    .then((result: { ok: boolean; username?: string; role?: string }) => {
      if (result.ok && result.username && result.role) {
...
export function useAuth() {
  ...
  return { isAuthenticated, isAdmin, currentUser, login, logout, authResolved, verifyToken }
}
