...
describe('useAuth', () => {
  ...
  describe('login', () => {
    ...
    it('returns message from server on rate limit', async () => {
      mockMutateAsync.mockResolvedValue({
        ok: false,
        message: 'Too many login attempts. Try again in 842s.',
      })

      const { login, isAuthenticated } = useAuth()
      const result = await login('admin', 'wrong')

      expect(result).toEqual({ ok: false, message: 'Too many login attempts. Try again in 842s.' })
      expect(isAuthenticated.value).toBe(false)
    })
