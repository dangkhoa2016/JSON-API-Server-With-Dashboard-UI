...
describe.each([
  ['Albums', Albums, 'Albums', ['title', 'userId']],
  ['Comments', Comments, 'Comments', ['name', 'email', 'body', 'postId']],
  ['Photos', Photos, 'Photos', ['title', 'url', 'thumbnailUrl', 'albumId']],
  ['Posts', Posts, 'Posts', ['title', 'body', 'userId']],
  ['Todos', Todos, 'Todos', ['title', 'completed', 'userId']],
])('%s.vue', (name, Component, expectedTitle, expectedFieldKeys) => {
  it('renders', () => {
    const wrapper = shallowMount(Component)
    expect(wrapper.findComponent({ name: 'CrudPage' }).exists()).toBe(true)
  })
  ...
  it('forwards fields prop to ResourcePage', () => {
    ...
    expect(rp.props('fields').map((f: { key: string }) => f.key)).toEqual(expectedFieldKeys)
  })
