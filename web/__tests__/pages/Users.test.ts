...
const { isAdminRef } = vi.hoisted(() => ({ isAdminRef: { __v_isRef: true, value: false } }))
vi.mock('@/composables/useAuth', () => ({
  useAuth: () => ({
    isAdmin: isAdminRef,
    isAuthenticated: { __v_isRef: true, value: false },
    currentUser: { __v_isRef: true, value: null },
    login: vi.fn(),
    logout: vi.fn(),
    authResolved: { __v_isRef: true, value: true },
    verifyToken: vi.fn(),
  }),
}))
...
describe('Users.vue', () => {
  ...
  it('gates delete for non-admin users', () => {
    isAdminRef.value = false
    const wrapper = shallowMount(Users)
    const rp = wrapper.findComponent({ name: 'ResourcePage' })
    expect(rp.props('canDelete')).toBe(false)
  })
  ...
  it('allows delete for admin users', () => {
    ...
    const wrapper = shallowMount(Users)
    const rp = wrapper.findComponent({ name: 'ResourcePage' })
    expect(rp.props('canDelete')).toBe(true)
  })
