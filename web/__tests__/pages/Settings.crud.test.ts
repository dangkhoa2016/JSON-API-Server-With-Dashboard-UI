...
import {
  authState, invalidateQueries, refetchFn,
  toastSuccess, toastError,
  updateMutate, resetMutate, resetDbMutate, mutationOpts,
  resetAllMocks,
...
describe('Settings.vue — login, edit, and mutation callbacks', () => {
  ...
  it('saveEdit rejects argon2 hash values for ADMIN_PASSWORD_HASH', () => {
    const wrapper = createWrapper()
    const vm = wrapper.vm as any
    vm.startEdit('ADMIN_PASSWORD_HASH', '$argon2id$v=19$m=19456,t=2,p=1$ELR9WQOOF3c3nUdGMf0PKA$Pv/0CUH8FT5VJxtAtxKJBwNpUvWNB5kl')
    vm.saveEdit('ADMIN_PASSWORD_HASH')
    expect(updateMutate).not.toHaveBeenCalled()
    expect(toastError).toHaveBeenCalledWith('Password hash detected. Please enter a plain text password instead.')
  })
  ...
  it('saveEdit accepts plain text values for ADMIN_PASSWORD_HASH', () => {
    ...
    vm.saveEdit('ADMIN_PASSWORD_HASH')
    expect(updateMutate).toHaveBeenCalledWith({ key: 'ADMIN_PASSWORD_HASH', value: 'newpassword' })
  })
  ...
  it('reset database mutation onSuccess with ok: false shows error toast', () => {
    createWrapper()
    mutationOpts.resetDb.onSuccess({ ok: false, message: 'Failed to reset database' })
  ...
  it('update mutation success triggers invalidateQueries', () => {
    createWrapper()
    mutationOpts.update.onSuccess({ ok: true })
    expect(invalidateQueries).toHaveBeenCalledWith({ queryKey: [{ subsystem: 'trpc', path: 'admin.settings.list' }] })
  })
