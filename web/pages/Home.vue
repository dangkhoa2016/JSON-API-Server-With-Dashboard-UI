...
const {
  ...
  isLoading,
  isError,
} = trpc.json.getCounts.useQuery(undefined, {
  staleTime: 30_000,
});
...
const { data: featureCardsData, isLoading: isFeatureCardsLoading } =
  trpc.json.getFeatureCards.useQuery();

const featureCards = computed<FeatureCard[]>(() =>
  (featureCardsData.value ?? []).map(card => ({
    key: card.key,
    label: card.label,
    description: card.description,
    icon: iconMap[card.icon] || Database,
    iconBg: card.iconBg || "bg-blue-100 dark:bg-blue-900/30",
    iconColor: card.iconColor || "text-blue-600 dark:text-blue-400",
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      <template v-if="isFeatureCardsLoading">
        ...
        >
          <div class="flex items-center gap-3 mb-3">
            <div
              class="w-10 h-10 bg-gray-200 dark:bg-gray-600 rounded-lg animate-pulse"
            />
            <div class="flex-1 space-y-2">
              <div
                class="h-4 w-28 bg-gray-200 dark:bg-gray-600 rounded animate-pulse"
              />
              <div
                class="h-4 w-16 bg-gray-200 dark:bg-gray-600 rounded animate-pulse"
              />
            </div>
          </div>
          <div class="space-y-2">
            <div
              class="h-3 w-full bg-gray-200 dark:bg-gray-600 rounded animate-pulse"
            />
            <div
              class="h-3 w-3/4 bg-gray-200 dark:bg-gray-600 rounded animate-pulse"
            />
          </div>
        </div>
      </template>
      <template v-else>
        ...
        >
          ...
          <p
            class="text-sm text-gray-500 dark:text-gray-400 whitespace-pre-line leading-relaxed"
          >
            {{ card.description }}
          </p>
        </div>
        ...
        >
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
              <div
                :class="`w-12 h-12 ${card.color} rounded-xl flex items-center justify-center`"
              >
              ...
              <div>
                ...
                <div v-else class="mt-1.5">
                  <div
                    class="h-7 w-16 bg-gray-200 dark:bg-gray-600 rounded animate-pulse"
                  />
                </div>
              </div>
            </div>
