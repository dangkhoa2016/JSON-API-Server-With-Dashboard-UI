...
import { Settings as SettingsIcon, Loader2, LogIn, LogOut, Trash2 } from '@lucide/vue'
...
const updateMutation = trpc.admin.settings.update.useMutation({
  onSuccess: (result) => {
    if (result.ok) {
      ...
      queryClient.invalidateQueries({ queryKey: [{ subsystem: 'trpc', path: 'admin.settings.list' }] })
...
function saveEdit(key: string) {
  ...
  if (value !== undefined) {
    ...
    if (key === 'ADMIN_PASSWORD_HASH' && value.startsWith('$argon2')) {
      toast.error('Password hash detected. Please enter a plain text password instead.')
      return
    }
