...
import { resetLoginRateLimit } from "../adminRouter";
...
beforeEach(async () => {
  ...
  await db.run(sql`INSERT INTO settings (key, value, type, label, description, "group", is_public) VALUES
    ...
    ('ADMIN_SESSION_SECRET', 'test-session-secret-key-for-testing', 'string', 'Admin Session Secret', 'HMAC signing key', 'auth', 0)
  `);
  if (adminHash) {
    await db.run(sql`INSERT INTO settings (key, value, type, label, description, "group", is_public) VALUES
...
describe("admin.settings", () => {
  ...
  it("list returns all settings when authenticated", async () => {
    ...
    expect(result[1].key).toBe("ADMIN_SESSION_SECRET");
    expect(result[2].key).toBe("ADMIN_USERNAME");
    expect(result[3].key).toBe("APP_SECRET");
    expect(result[4].key).toBe("REDIS_ENABLED");
  });
