...
describe("admin.settings", () => {
  it("list returns only public settings when not authenticated", async () => {
    ...
    expect(result.every((s: any) => s.isPublic)).toBe(true);
    expect(result.find((s: any) => s.key === "APP_SECRET")).toBeUndefined();
    expect(result.find((s: any) => s.key === "REDIS_ENABLED")).toBeDefined();
  });
