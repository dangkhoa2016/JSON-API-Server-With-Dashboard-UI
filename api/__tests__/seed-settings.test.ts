...
describe("seedSettings", () => {
  ...
  it("inserts settings for env vars that are set", async () => {
    ...
    const rateLimitMax = rows.find(
      (r: any) => r.key === "RATE_LIMIT_MAX_REQUESTS"
    );
  ...
  it("uses default values for env vars that are not set", async () => {
    ...
    expect(logSpy).toHaveBeenCalledWith(
      "  Setting: DEBUG_SQL = false (default)"
    );
    expect(logSpy).toHaveBeenCalledWith("  Setting: REDIS_TTL = 300 (default)");
  ...
  it("handles onConflictDoNothing for duplicate keys", async () => {
    ...
    const rows = (await db.all(
      sql`SELECT * FROM settings WHERE key = 'REDIS_HOST'`
    )) as any[];
    expect(rows.length).toBe(1);
    expect(rows[0].value).toBe("first-value");
  });

  it("seeds REDIS_URL with empty default when env unset", async () => {
    ...
    const rows = (await db.all(
      sql`SELECT * FROM settings WHERE key = 'REDIS_URL'`
    )) as any[];
    expect(rows.length).toBe(1);
