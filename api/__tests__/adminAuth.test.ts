...
describe('adminAuth', () => {
  ...
  it('createSession throws when APP_SECRET is not in database', async () => {
    const { getDb } = await import('../queries/connection')
    const { sql } = await import('drizzle-orm')
    const db = getDb()
    await db.run(sql`DELETE FROM settings WHERE key = 'APP_SECRET'`)
    vi.setSystemTime(Date.now() + 120_000)
    await expect(createSession('admin')).rejects.toThrow('APP_SECRET not configured')
  })
})
