...
describe("env", () => {
  ...
  it("throws when APP_SECRET is missing", async () => {
    ...
    await expect(async () => {
      await import("../lib/env");
    }).rejects.toThrow("Missing required environment variable: APP_SECRET");
  });
  ...
  it("handles optionalBool with '0' string", async () => {
    process.env.APP_SECRET = "test";
    process.env.DATABASE_URL = ":memory:";
  ...
  it("handles optionalBool with 'no' string", async () => {
    process.env.APP_SECRET = "test";
    process.env.DATABASE_URL = ":memory:";
  ...
  it("returns default for optional string when missing", async () => {
    process.env.APP_SECRET = "test";
    process.env.DATABASE_URL = ":memory:";
  ...
  it("reads optional string when present", async () => {
    process.env.APP_SECRET = "test";
    process.env.DATABASE_URL = ":memory:";
    process.env.REDIS_HOST = "custom-host";
    const { env } = await import("../lib/env");
    expect(env.redisHost).toBe("custom-host");
  });

  it("returns empty string for optional string with empty value", async () => {
    process.env.APP_SECRET = "test";
    process.env.DATABASE_URL = ":memory:";
    process.env.REDIS_HOST = "";
    const { env } = await import("../lib/env");
    expect(env.redisHost).toBe("");
  });

  it("reads REDIS_PASSWORD as optional string", async () => {
    process.env.APP_SECRET = "test";
    process.env.DATABASE_URL = ":memory:";
    process.env.REDIS_PASSWORD = "secret";
    const { env } = await import("../lib/env");
    expect(env.redisPassword).toBe("secret");
  });

  it("defaults REDIS_PASSWORD to empty string", async () => {
    process.env.APP_SECRET = "test";
    process.env.DATABASE_URL = ":memory:";
    delete process.env.REDIS_PASSWORD;
    const { env } = await import("../lib/env");
    expect(env.redisPassword).toBe("");
  });

  it("parses RATE_LIMIT_WINDOW_MS", async () => {
    process.env.APP_SECRET = "test";
    process.env.DATABASE_URL = ":memory:";
    process.env.RATE_LIMIT_WINDOW_MS = "30000";
    const { env } = await import("../lib/env");
    expect(env.rateLimitWindowMs).toBe(30000);
  });

  it("parses RATE_LIMIT_MAX_REQUESTS", async () => {
    process.env.APP_SECRET = "test";
    process.env.DATABASE_URL = ":memory:";
  ...
  it("parses CACHE_TTL_SECONDS", async () => {
    process.env.APP_SECRET = "test";
    process.env.DATABASE_URL = ":memory:";
    process.env.CACHE_TTL_SECONDS = "600";
    const { env } = await import("../lib/env");
    expect(env.cacheTtlSeconds).toBe(600);
  });

  it("defaults isProduction to false", async () => {
    process.env.APP_SECRET = "test";
    process.env.DATABASE_URL = ":memory:";
  ...
  it("defaults DEBUG_SQL to false", async () => {
    process.env.APP_SECRET = "test";
    process.env.DATABASE_URL = ":memory:";
    delete process.env.DEBUG_SQL;
    const { env } = await import("../lib/env");
    expect(env.debugSql).toBe(false);
  });

  it("parses DEBUG_SQL as true", async () => {
    process.env.APP_SECRET = "test";
    process.env.DATABASE_URL = ":memory:";
    process.env.DEBUG_SQL = "true";
    const { env } = await import("../lib/env");
    expect(env.debugSql).toBe(true);
  });
});
