...
describe("admin.settings.update", () => {
  ...
  it("rejects empty values", async () => {
    const caller = createAdminCaller();
    const result = await caller.settings.update({ key: "REDIS_ENABLED", value: " " });
    expect(result.ok).toBe(false);
  ...
  it("allows all-asterisk values with force", async () => {
    const caller = createAdminCaller();
    const result = await caller.settings.update({ key: "REDIS_ENABLED", value: "***", force: true });
    expect(result.ok).toBe(true);
  });

  it("hashes plain text password when updating ADMIN_PASSWORD_HASH", async () => {
    const caller = createAdminCaller();
    const result = await caller.settings.update({ key: "ADMIN_PASSWORD_HASH", value: "newpassword" });
    expect(result.ok).toBe(true);
    const db = getDb();
    const row = await db.select().from(schema.settings).where(eq(schema.settings.key, "ADMIN_PASSWORD_HASH")).get();
  ...
  it("rejects argon2 hash values for ADMIN_PASSWORD_HASH", async () => {
    const caller = createAdminCaller();
    const existingHash = (await getDb().select().from(schema.settings).where(eq(schema.settings.key, "ADMIN_PASSWORD_HASH")).get())!.value;
    const result = await caller.settings.update({ key: "ADMIN_PASSWORD_HASH", value: existingHash });
    expect(result.ok).toBe(false);
  ...
  it("login works after updating ADMIN_PASSWORD_HASH with plain text", async () => {
    ...
    await adminCaller.settings.update({ key: "ADMIN_PASSWORD_HASH", value: "newpassword" });
    const result = await caller.auth.login({ username: "admin", password: "newpassword" });
    expect(result.ok).toBe(true);
  });
});
