...
describe("admin auth", () => {
  it("login fails with wrong username (timingSafeEqual length mismatch)", async () => {
    const caller = appRouter.createCaller({
      req: new Request("http://test.com"),
      resHeaders: new Headers(),
    });
    const result = await caller.admin.auth.login({ username: "admin", password: "wrong" });
  ...
  it("login fails with wrong username", async () => {
    ...
    const result = await caller.admin.auth.login({ username: "wronguser", password: "pass" });
    expect(result.ok).toBe(false);
  ...
  it("login fails with wrong password", async () => {
    ...
    const result = await caller.admin.auth.login({ username: "admin", password: "wrongpassword" });
    expect(result.ok).toBe(false);
    expect(result.message).toBe("Invalid username or password");
  });

  it("login fails with username of different length", async () => {
    const caller = appRouter.createCaller({
      req: new Request("http://test.com"),
      resHeaders: new Headers(),
    });
    const result = await caller.admin.auth.login({ username: "adminadmin", password: "pass" });
    expect(result.ok).toBe(false);
  });

  it("verify returns false without token", async () => {
    const caller = appRouter.createCaller({
      req: new Request("http://test.com"),
      resHeaders: new Headers(),
    });
  ...
  it("verify returns true with valid token", async () => {
  ...
  it("verify returns false with invalid token", async () => {
    const caller = appRouter.createCaller({
      req: new Request("http://test.com", {
  ...
  it("verify returns false with non-Bearer auth", async () => {
    const caller = appRouter.createCaller({
      req: new Request("http://test.com", {
...
describe("admin settings", () => {
  it("list returns public settings for non-admin", async () => {
    const caller = appRouter.createCaller({
      req: new Request("http://test.com"),
      resHeaders: new Headers(),
    });
  ...
  it("list returns all settings for admin", async () => {
    const { createSession } = await import("../lib/adminAuth");
    const token = createSession("admin");
    const caller = appRouter.createCaller({
      req: new Request("http://test.com", {
  ...
  it("getByKey returns null for non-existent key", async () => {
    ...
    const result = await caller.admin.settings.getByKey({ key: "NONEXISTENT" });
    expect(result).toBeNull();
  });

  it("getByKey returns public setting for non-admin", async () => {
    const caller = appRouter.createCaller({
      req: new Request("http://test.com"),
      resHeaders: new Headers(),
    });
    const result = await caller.admin.settings.getByKey({ key: "APP_NAME" });
  ...
  it("update requires admin auth", async () => {
    ...
    await expect(caller.admin.settings.update({ key: "APP_NAME", value: "test" })).rejects.toThrow();
  });

  it("update fails for non-existent key", async () => {
    const { createSession } = await import("../lib/adminAuth");
    const token = createSession("admin");
    const caller = appRouter.createCaller({
      req: new Request("http://test.com", {
    ...
    const result = await caller.admin.settings.update({ key: "NONEXISTENT", value: "test" });
    expect(result.ok).toBe(false);
  });

  it("update succeeds for existing key", async () => {
    const { createSession } = await import("../lib/adminAuth");
    const token = createSession("admin");
    const caller = appRouter.createCaller({
      req: new Request("http://test.com", {
    ...
    const result = await caller.admin.settings.update({ key: "APP_NAME", value: "NewName" });
    expect(result.ok).toBe(true);
  });

  it("reveal requires admin auth", async () => {
    const caller = appRouter.createCaller({
      req: new Request("http://test.com"),
      resHeaders: new Headers(),
    });
    await expect(caller.admin.settings.reveal({ key: "APP_NAME" })).rejects.toThrow();
  });

  it("reveal returns setting value for admin", async () => {
    const { createSession } = await import("../lib/adminAuth");
    const token = createSession("admin");
    const caller = appRouter.createCaller({
      req: new Request("http://test.com", {
    ...
    const result = await caller.admin.settings.reveal({ key: "APP_NAME" });
    expect(result).toBeDefined();
  ...
  it("reveal returns null for non-existent key", async () => {
    ...
    const result = await caller.admin.settings.reveal({ key: "NONEXISTENT" });
    expect(result).toBeNull();
  });

  it("reset requires admin auth", async () => {
    const caller = appRouter.createCaller({
      req: new Request("http://test.com"),
      resHeaders: new Headers(),
    });
    await expect(caller.admin.settings.reset({ key: "APP_NAME" })).rejects.toThrow();
  });

  it("reset fails for non-resetable key", async () => {
    const { createSession } = await import("../lib/adminAuth");
    const token = createSession("admin");
    const caller = appRouter.createCaller({
      req: new Request("http://test.com", {
    ...
    const result = await caller.admin.settings.reset({ key: "NONEXISTENT_KEY" });
    expect(result.ok).toBe(false);
  });

  it("reset fails when no env value available", async () => {
    const { createSession } = await import("../lib/adminAuth");
    const token = createSession("admin");
    const caller = appRouter.createCaller({
      req: new Request("http://test.com", {
    ...
    const result = await caller.admin.settings.reset({ key: "ADMIN_USERNAME" });
    expect(result.ok).toBe(false);
    expect(result.message).toContain("No environment value");
  });

  it("reset succeeds for allowed key with env value", async () => {
    const { createSession } = await import("../lib/adminAuth");
    const token = createSession("admin");
    const caller = appRouter.createCaller({
      req: new Request("http://test.com", {
    ...
    const result = await caller.admin.settings.reset({ key: "APP_SECRET" });
    expect(result.ok).toBe(true);
  });
});

describe("admin data", () => {
  it("seed requires admin auth", async () => {
    const caller = appRouter.createCaller({
      req: new Request("http://test.com"),
      resHeaders: new Headers(),
    });
  ...
  it("seed works for admin", async () => {
    ...
    const result = await caller.admin.data.seed();
    expect(result.ok).toBe(true);
  });

  it("resetDatabase requires admin auth", async () => {
    const caller = appRouter.createCaller({
      req: new Request("http://test.com"),
      resHeaders: new Headers(),
    });
    await expect(caller.admin.data.resetDatabase()).rejects.toThrow();
  });

  it("resetDatabase works for admin", async () => {
    const { createSession } = await import("../lib/adminAuth");
    const token = createSession("admin");
    const caller = appRouter.createCaller({
      req: new Request("http://test.com", {
    ...
    const result = await caller.admin.data.resetDatabase();
    expect(result.ok).toBe(true);
  });
});
