...
describe("body size limit", () => {
  ...
  it("rejects oversized streaming body without content-length", async () => {
    ...
    miniApp.post("/test", bodyLimit({ maxBytes: 100 }), async (c) => c.json({ ok: true }, 201));
  ...
  it("allows small JSON body within limit", async () => {
    const res = await app.request("/api/users", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ name: "Test", username: "testuser", email: "test@test.com" }),
    });
    expect(res.status).toBe(201);
  });
  ...
  it("rejects non-numeric content-length", async () => {
    ...
    expect(res.status).toBe(400);
    expect(await res.json()).toMatchObject({ error: "Request body unreadable" });
  });
