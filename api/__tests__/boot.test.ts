...
import app, { contextClientIp } from "../boot";
import { getDb } from "../queries/connection";
import { bodyLimit } from "../lib/bodyLimit";
...
type JsonBody = Record<string, unknown> & {
  ...
  result: { data: { json: { ok: boolean; ts?: number } } };
};
...
describe("REST API - /api/:resource", () => {
  describe("GET /api/:resource", () => {
    ...
    it("returns 400 for a non-numeric limit", async () => {
      const res = await app.request("/api/users?_limit=abc");
...
describe("admin REST error", () => {
  it("returns 401 for unauthorized request to protected route", async () => {
  ...
  it("returns 400 for invalid input on login", async () => {
  ...
  it("does not leak internal error details", async () => {
    ...
    expect(text).not.toContain("TRPCError");
...
describe("Admin REST routes", () => {
  ...
  it("PUT /api/admin/settings/:key rejects all-asterisk values without force", async () => {
    await seedAdminCreds();
    const token = await createSession("admin");
    const db = getDb();
    await db.run(sql`INSERT INTO settings (key, value, type, label, description, "group", is_public)
      VALUES ('AST_TEST', 'initial', 'string', 'Test', 'Test setting', 'general', 0)`);

    const res = await app.request("/api/admin/settings/AST_TEST", {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
    ...
    const body = (await res.json()) as JsonBody;
    expect(body.ok).toBe(false);
    expect(body.message).toContain("all asterisks");
  });

  it("PUT /api/admin/settings/:key allows all-asterisk values with force", async () => {
    await seedAdminCreds();
    const token = await createSession("admin");
    const db = getDb();
    await db.run(sql`INSERT INTO settings (key, value, type, label, description, "group", is_public)
      VALUES ('FORCE_TEST', 'initial', 'string', 'Test', 'Test setting', 'general', 0)`);

    const res = await app.request("/api/admin/settings/FORCE_TEST", {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
      },
      body: JSON.stringify({ value: "***", force: true }),
    });
    expect(res.status).toBe(200);
    const body = (await res.json()) as JsonBody;
    expect(body.ok).toBe(true);
...
describe("body size limit", () => {
  ...
  it("rejects oversized streaming body without content-length", async () => {
    const { bodyLimit } = await import("../lib/bodyLimit");
    const miniApp = new Hono();
    miniApp.post("/test", bodyLimit({ maxBytes: 100 }), async c =>
      c.json({ ok: true }, 201)
    );

    const largeBody = new Uint8Array(200);
    const res = await miniApp.request("/test", {
      method: "POST",
      body: largeBody,
    });
    expect(res.status).toBe(413);
  ...
  it("allows small JSON body within limit", async () => {
    const res = await app.request("/api/users", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
  ...
  it("allows body at exact limit boundary", async () => {
    const { bodyLimit } = await import("../lib/bodyLimit");
    const miniApp = new Hono();
    miniApp.post("/test", bodyLimit({ maxBytes: 100 }), async c =>
      c.json({ ok: true }, 201)
    ...
    const okBody = new Uint8Array(100);
    const res = await miniApp.request("/test", {
      method: "POST",
      body: okBody,
    });
  ...
  describe("contextClientIp", () => {
    const trustedPeer = "10.0.0.4";
    const untrustedPeer = "203.0.113.10";
    const firstClient = "198.51.100.20";
    const secondClient = "198.51.100.21";

    function mockContext(remoteAddress: string, xff: string | null) {
      return {
        req: {
          header: (name: string) => (name === "x-forwarded-for" ? xff : null),
          raw: { headers: new Map(xff ? [["x-forwarded-for", xff]] : []) },
        },
        env: { incoming: { socket: { remoteAddress } } },
      } as never;
    }

    it("ignores XFF from an untrusted peer", () => {
      const ip = contextClientIp(mockContext(untrustedPeer, `${firstClient}`));
      expect(ip).toBe(untrustedPeer);
    });

    it("resolves distinct clients behind a trusted proxy", () => {
      const ip1 = contextClientIp(mockContext(trustedPeer, `${firstClient}`));
      const ip2 = contextClientIp(mockContext(trustedPeer, `${secondClient}`));
      expect(ip1).toBe(firstClient);
      expect(ip2).toBe(secondClient);
    });
  });

  it("rejects request with unreadable body stream", async () => {
    const { bodyLimit } = await import("../lib/bodyLimit");
    const miniApp = new Hono();
    miniApp.post("/test", bodyLimit({ maxBytes: 100 }), async c =>
      c.json({ ok: true }, 201)
  ...
  it("rejects request when cloned body stream is unavailable", async () => {
    const middleware = bodyLimit({ maxBytes: 100 });
    const c = {
      req: {
        method: "POST",
        header: () => undefined,
    ...
    const res = await middleware(c, vi.fn() as any);
    expect((res as any).status).toBe(400);
    expect((res as any).body).toEqual({ error: "Request body unreadable" });
  });
});
...
describe("REST API - error handling", () => {
  ...
  it("returns an error response when getById fails", async () => {
    vi.mocked(getDb).mockImplementation(() => {
      throw new TRPCError({ code: "BAD_REQUEST", message: "db failure" });
    });
    const res = await app.request("/api/users/1");
    expect(res.status).toBe(400);
    const body = (await res.json()) as JsonBody;
  ...
  it("returns an error response when delete fails", async () => {
    vi.mocked(getDb).mockImplementation(() => {
      throw new TRPCError({ code: "BAD_REQUEST", message: "db failure" });
    });
    const res = await app.request("/api/users/1", { method: "DELETE" });
