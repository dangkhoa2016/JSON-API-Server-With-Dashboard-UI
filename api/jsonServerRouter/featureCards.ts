...
export async function getFeatureCards(): Promise<FeatureCardRow[]> {
  ...
  if (referenced.size > 0) {
    ...
    const replaceRefs = (text: string) =>
      text.replace(/\{\{(\w+)\}\}/g, (_, k: string) => valueByKey[k] ?? process.env[k] ?? defaults[k] ?? `{{${k}}}`);
  ...
  for (const card of raw) {
    if (card.key === "feature_card_redis") {
      card.healthy = (valueByKey.REDIS_ENABLED ?? process.env.REDIS_ENABLED) === "true";
    } else if (card.key === "feature_card_ratelimit") {
      card.healthy = (valueByKey.RATE_LIMIT_ENABLED ?? process.env.RATE_LIMIT_ENABLED) === "true";
    }
  }
