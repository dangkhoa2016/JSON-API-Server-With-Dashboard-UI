...
import { getClientIp } from "./lib/ratelimit/ipUtils";
...
export function contextClientIp(c: Context): string {
  return getClientIp(c, env.trustedProxyCidrs);
}
...
function getJsonCaller(c: Context) {
  ...
  return {
    ...
    getById: (resource: Resource, id: number) =>
      handlers[resource].getById({ id }),
    create: (resource: Resource, body: unknown) =>
      handlers[resource].create(body),
    update: (resource: Resource, id: number, body: unknown) =>
      handlers[resource].update({ id, data: body }),
    delete: (resource: Resource, id: number) =>
      handlers[resource].delete({ id }),
  };
}
...
app.use(bodyLimit({ maxBytes: 50 * 1024 * 1024 }));
...
app.use("/api/trpc/*", async c => {
  const clientIp = contextClientIp(c);
...
async function adminCall(
  c: Context,
  fn: (admin: ReturnType<typeof getAdminCaller>) => Promise<Response>
): Promise<Response> {
  try {
...
app.put("/api/admin/settings/:key", async c => {
  return adminCall(c, async admin => {
    const { value, force } = await c.req.json();
...
app.get("/api/:resource/:id", async c => {
  try {
    ...
    if (!validateResource(resource) || isNaN(id))
