...
async function insertSeedData(tx: SeedDb) {
  const BASE = "https://jsonplaceholder.typicode.com";

  const seedUsers = await fetchWithRetry(`${BASE}/users`).then((r) => r.json()) as JsonUser[];
  ...
  const seedPosts = await fetchWithRetry(`${BASE}/posts`).then((r) => r.json()) as JsonPost[];
  ...
  const seedComments = await fetchWithRetry(`${BASE}/comments`).then((r) => r.json()) as JsonComment[];
  ...
  const seedAlbums = await fetchWithRetry(`${BASE}/albums`).then((r) => r.json()) as JsonAlbum[];
  ...
  const seedPhotos = await fetchWithRetry(`${BASE}/photos`).then((r) => r.json()) as JsonPhoto[];
  ...
  const seedTodos = await fetchWithRetry(`${BASE}/todos`).then((r) => r.json()) as JsonTodo[];
...
export async function replaceSeedData(tx: SeedDb, resetSequence?: boolean) {
  await tx.delete(schema.photos).run();
  await tx.delete(schema.comments).run();
  await tx.delete(schema.posts).run();
  await tx.delete(schema.albums).run();
  await tx.delete(schema.todos).run();
  await tx.delete(schema.users).run();
...
export async function seedDatabase(dbInstance: SeedDb) {
  ...
  await insertSeedData(dbInstance);
}
