...
- **Coverage-enforced test suite** — **69 test files** across backend and frontend with vitest coverage thresholds (80% statements/functions/lines, 75% branches). Integration tests run against real SQLite + HTTP; component tests use `@vue/test-utils` with jsdom; E2E browser tests use Playwright.
...
| `yarn test:coverage`    | Run tests with coverage (thresholds: 80/75/80/80)         |
...
│   └── __tests__/              # Backend test suite (27 files)
├── db/
│   ├── schema.js               # Drizzle schema (7 tables)
│   ├── relations.ts            # Table relations
│   ├── seed.js                 # Seed from JSONPlaceholder
│   ├── seed-settings.js        # Seed settings
│   ├── seed-admin.js           # Seed admin credentials
...
│   └── __tests__/              # Frontend test suite (41 files)
...
The project enforces **coverage thresholds** via vitest (statements 80%, branches 75%, functions 80%, lines 80%). The test suite comprises:
...
- **Database tests** (2 files, `node` environment): Tests covering Drizzle config, schema preparation, and database seeding.
- **Scripts tests** (1 file, `node` environment): Tests for the interactive database inspection tool.
...
See [manual/README.md](manual/README.md) for the full script reference and usage details.
